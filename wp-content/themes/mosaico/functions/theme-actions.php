...
function themnific_custom_styling() {
	...
	if ($body_color)
		$output = 'body,#navigation .nav li ul,ul#serinfo,#serinfo-nav li.current{background-color:'.$body_color.'}' . "\n";
	if ($sec_body_color)
		$output .= '
	...
	if ($link)
		$output .= 'a:link, a:visited,#serinfo h4 a,#navigation .nav li ul li a {color:'.$link.'}' . "\n";
	if ($link_alter)
		$output .= '.body3 a {color:'.$link_alter.'}' . "\n";
	if ($hover)
		$output .= 'a:hover,#serinfo a:hover,
		...
		#navigation .nav li ul li a:hover {color:'.$hover.'}' . "\n";
	if ($sec_link)
		$output .= '
	...
	if ($body_bg)
		$output .= '
		body,.heading-text
		{background-image:url('.$home.'/wp-content/themes/thiel/images/bg/'.$body_bg.')}' . "\n";
		
		
	if ($border)
		$output .= '#tagline,.bags,#sidebar h2,ul#serinfo-nav li,#serinfo,ul.medpost li p,#sidebar .fblock,.searchform input.s {border-color:'.$border.' !important}' . "\n";	

	if ($border_sec)
		$output .= 'ul.bigpost li.alter .bigmeta,ul.bigpost li.alter {border-color:'.$border_sec.' !important}' . "\n";

	if ($shadows)
		$output .= 'body,.ei-slider-thumbs li a {text-shadow:0 1px 1px '.$shadows.'}' . "\n";
		
	if ($shadows_sec)
		$output .= '.body2 {text-shadow:0 1px 1px '.$shadows_sec.'}' . "\n";
		
	if ($shadows_thi)
		$output .= '.body3,.ei-title h2 {text-shadow:0 1px 1px '.$shadows_thi.'}' . "\n";
		...
		if ( $font_text_sec )
			$output .= '.body2, .body2#toggleArchives {font:'.$font_text_sec["style"].' '.$font_text_sec["size"].'px/2.2em '.stripslashes($font_text_sec["face"]).';color:'.$font_text_sec["color"].'}' . "\n";
